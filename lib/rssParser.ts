...
export async function fetchFeed(
  url: string,
): Promise<Array<{ title: string; link: string; pubDate: Date; snippet: string }>> {
...
function parseItem(chunk: string, isAtom: boolean): FeedItem {
  ...
  const rawSnippet =
    ...
    ''
  const snippet = normalizeFeedText(stripHtml(decodeEntities(rawSnippet))).slice(0, 400)
...
export function normalizeFeedText(str: string): string {
  let out = str

  try {
    out = decodeURIComponent(escape(out))
  } catch {
    // Leave the original text alone if this is not mojibake.
  }
